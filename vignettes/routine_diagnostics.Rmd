...
In brief, `smdi_diagnose()` computes three group diagnostics.
...
A great way to explore the patterns of missingness is the `md.pattern` function which is part of the `mice` package. 

>We recommend checking the missing data patterns as part of the routine missing data workflow using the `smdi` package. It may provide important information about potential monotone missing data patterns. In case of monotonocity, the `smdi` package may likely culminate in misleading results. Please check the [article on multivariate missingness and monotonicity](https://janickweberpals.gitlab-pages.partners.org/smdi/articles/multivariate_missingness.html)

```{r, fig.width = 8, fig.height=6, fig.width = 10}
mice::md.pattern(smdi_data, rotate.names = TRUE)
```
