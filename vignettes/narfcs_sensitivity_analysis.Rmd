...
smdi_data <- smdi::smdi_data_complete %>% 
  # and one hot encode the SES variable
...
```{r}
# true outcome model
cox_fit_true <- survival::coxph(cox_form, data = smdi_data) %>% 
...
As we can observe, the complete case analysis and the multiple imputation approach are off.
...
narfcs_range_df %>% 
  ...
  ggplot2::scale_x_continuous(breaks = seq(-50, 50, 1), limits = c(-50, 50)) +
  ggplot2::scale_y_continuous(breaks = seq(0.6, 1.1, 0.1), limits = c(0.6, 1.1)) +
