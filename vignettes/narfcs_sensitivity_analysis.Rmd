...
As mentioned above, the true (but unobserved) values of a variable systematically differ from the observed values and this difference is reflected in the sensitivity parameter **𝛅**. More specifically, the interpretation of **𝛅** would be the difference in the distribution of missing and observed `age_num` values conditional on other covariates. Hence, for continuous covariates (like in our example) it would be the difference in mean conditional on `r toString(paste0(covariates[-1], collapse = " + "))`. Since our data generation model did not specifiy any interactions or relationships between covariates, we can simplify this here a little and just take the crude difference in means.
...
```
Based on this tipping point analysis, we would have figured that a systematic departure of `age_num` from MAR may be very likely in our main anlysis, suggesting that the primary analysis estimates for `exposure` are likely biased by a not at random value (MNAR[value]) missingness of `age_num`.
...
Alternatively, we can use `smdi_vis()`.

```{r}
smdi::smdi_data %>% 
  smdi::smdi_vis(top_n_covar = 3)
```

## `smdi_diagnose` analysis
...
As we can see, also `pdl1_num` is sensitive to departures from MAR under small, realistic **𝛅** sensitivity parameter values.
