...
forest <- dplyr::bind_rows(cox_fit_true, cox_fit_cc, cox_fit_imp) %>% 
  ...
  ggplot2::geom_point(aes(x = estimate), shape = 15, size = 3) +
  ggplot2::geom_errorbar(aes(xmin = conf.low, xmax = conf.high)) +
...
As we can observe, the complete case analysis slightly overestimates the true treatment effect while the multiple imputation approach slightly underestimates it. However, the confidence intervals are highly overlapping and the bias is limited given the rather weak association between age and exposure (~ OR 0.97) and age and the outcome (~ HR 1.05).

Nevertheless, let's assume we wouldn't know this as in reality we actually don't. We want to make sure that whatever analytical decision we would have chosen for our primary analysis (be it a complete case approach or an imputation approach), our results would not drastically change if the true underlying missingness mechanism was MNAR(value).

This is where the NARFCS sensitivity analysis comes in handy as it is also often used as a **tipping point analysis**: we model multiple NARFCS-specified imputation models over a range of𝛅sensitivity parameters and evaluate if at any pre-specified 𝛅, our confidence interval would cross a certain estimate threshold which would discard the qualitative conclusion of our main analysis. 

The interpretation of𝛅would be the difference in the distribution of missing and observed age values conditioned on other baseline factors. More specifically, for continuous covariates (like in our example) it would be the difference in mean conditional on `r toString(paste0(covariates[-1], collapse = " + "))`
...
mnar_blot <- list(age_num = list(ums = "-5.144647"))

imp_narfcs <- mice::mice(
  data = smdi_data_mnar_v,
...
narfcs_range_df %>% 
  ...
  ggplot2::geom_smooth() +
  ggplot2::labs(x = "Sensitivity parameter δ", y = "Effect estimate, e.g. Hazard ratio (95% CI)") +
  #ggplot2::scale_x_continuous(breaks = seq(-5, 5, 1), limits = c(-5, 5)) +
  #ggplot2::scale_y_continuous(breaks = seq(.6, 3.4, 0.2), limits = c(.6, 3.4)) +
  ggplot2::geom_hline(yintercept = estimate_true, linetype = "dashed", color = "red") +
  ggplot2::theme_bw()
```
