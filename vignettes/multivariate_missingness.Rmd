...
mice::md.pattern(data_monotone[, c("lab1", "lab2")], rotate.names = TRUE)
```
...
gt(diagnostics_jointly$smdi_tbl)
diagnostics_jointly$p_little
```
...
gt(lab1_diagnostics$smdi_tbl)
lab1_diagnostics$p_little
```
...
gt(lab2_diagnostics$smdi_tbl)
lab2_diagnostics$p_little
```

Since the missingness follows an MCAR mechanism (we simply set all values in the first 500 rows to NA), `smdi_diagnose()` now again reliably shows the common missingness diagnostics patterns as expected.
